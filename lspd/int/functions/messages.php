...
if ($type == "reply") {
    $req = $bdd->prepare('SELECT sender, title, sender_type FROM messages_lspd WHERE ID = ?');
...
if ($type == "delete"){
    $req = $bdd->prepare('DELETE FROM messages_lspd WHERE ID = ?');
    $req->execute(array($ID));
    echo "success";
}

if ($type == "deleteall"){
    $req = $bdd->prepare('DELETE FROM messages_lspd WHERE receiver = ?');
    $req->execute(array($ID));
    echo "success";
}

if ($type == "markasread"){
    $req = $bdd->prepare('UPDATE messages_lspd SET markasread = 1 WHERE ID = ?');
    $req->execute(array($ID));
    echo "success";
}

if ($type == "markallasread"){
    $req = $bdd->prepare('UPDATE messages_lspd SET markasread = 1 WHERE receiver = ?');
    $req->execute(array($ID));
    echo "success";
}
