...
<ul class="navbar-nav ml-auto">
            ...
            if ($count > 0) {
                foreach ($data as $notif){
                    echo '<a class="dropdown-item d-flex align-items-center"><div class="mr-3">';
                        ...
                        }elseif ($notif["type"] == "fine"){
                            echo '<div class="icon-circle bg-danger"><i class="fas fa-money-bill-wave text-white"></i></div>';
                        }
                        echo '</div>
                        <div>
    ...
    $reqmessage = $bdd->prepare('SELECT * FROM messages_lspd WHERE receiver = ? AND markasread = 0 ORDER BY datetime DESC');
